  ...
  req.pool.getConnection(function(err, connection){
    ...
    connection.query(checkUserID, [req.body.userID], function(err, rows, fields){
      ...
      if(err){
        ...
        res.json({success: false, error: 'Account does not exist! Please check your username or sign up for a new account.'});
        return;
      }
    ...
    connection.query(query, [req.body.userID], function(err, rows, fields){
      ...
      bcrypt.compare(input, correctPassword, function(err, result){
        if(result){
          ...
          res.json({success: true, accountType: accountType});
        }
        else{
          res.json({success: false, error: 'Incorrect password! Please check your password and try again.'});
          return;
        }
      })
    })
    ...
    req.pool.getConnection(function(err, connection){
      ...
      connection.query(query, function(err, rows, fields){
        ...
        async function verify() {
          ...
          if(accountIndex === -1){
            res.json({success: false, error: 'Google account not found! Please sign up first or use regular login.'});
          }
          else{
            // res.json(rows[accountIndex]);
